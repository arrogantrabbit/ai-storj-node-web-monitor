...
# --- Background Tasks ---
# THE FINAL, ROBUST, DELTA-BASED BANDWIDTH CALCULATOR
async def bandwidth_calculator(app):
    # This variable will remember the timestamp of the last event processed.
    ...
    while True:
        ...
        # On the first run, initialize the timestamp to avoid calculating the entire deque.
        if last_processed_ts == 0.0:
            last_processed_ts = app_state['live_events'][-1]['ts_unix']

        newest_ts_in_batch = last_processed_ts
        
        # Iterate over new events that have arrived since the last time we ran.
        for event in reversed(app_state['live_events']):
            if event['ts_unix'] <= last_processed_ts:
                break
            
            if 'GET' in event['action']: egress_bytes += event['size']
            else: ingress_bytes += event['size']
            
            if event['ts_unix'] > newest_ts_in_batch:
                 newest_ts_in_batch = event['ts_unix']
        
        # Remember the timestamp of the newest event for the next iteration.
