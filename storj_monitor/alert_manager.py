...
class AlertManager:
    ...
    async def generate_alert(
        ...
        try:
            ...
            success = await loop.run_in_executor(
                self.app["db_executor"], blocking_write_alert, DATABASE_FILE, alert
            )
    ...
    async def evaluate_storage_alerts(self, node_name: str, storage_data: dict[str, Any]):
    ...
    async def evaluate_latency_alerts(self, node_name: str, latency_data: dict[str, Any]):
    ...
    async def acknowledge_alert(self, alert_id: int) -> bool:
        ...
        try:
            ...
            success = await loop.run_in_executor(
                self.app["db_executor"], blocking_acknowledge_alert, DATABASE_FILE, alert_id
            )
            ...
            if success:
                ...
                await robust_broadcast(
                    app_state["websockets"], {"type": "alert_acknowledged", "alert_id": alert_id}
                )
    ...
    async def get_active_alerts(self, node_names: list[str] = None) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            alerts = await loop.run_in_executor(
                self.app["db_executor"], blocking_get_active_alerts, DATABASE_FILE, node_names
            )
    ...
    def get_alert_summary(self) -> dict[str, int]:
        ...
        summary = {"critical": 0, "warning": 0, "info": 0, "total": len(self.active_alerts)}
...
async def alert_evaluation_task(app):
    ...
    while True:
        try:
            ...
            from .config import DATABASE_FILE
            from .database import blocking_get_latest_reputation, blocking_get_latest_storage
            from .state import app_state
            ...
            for node_name in node_names:
                try:
                    ...
                    if reputation_data:
                        await alert_manager.evaluate_reputation_alerts(node_name, reputation_data)

                        # Analyze reputation health
                        insights = await analytics.analyze_reputation_health(
                            node_name, reputation_data
                        )
                    ...
                    storage_data_list = await loop.run_in_executor(
                        app["db_executor"], blocking_get_latest_storage, DATABASE_FILE, [node_name]
                    )

                    if storage_data_list:
                        storage_data = storage_data_list[0]

                        await alert_manager.evaluate_storage_alerts(node_name, storage_data)
