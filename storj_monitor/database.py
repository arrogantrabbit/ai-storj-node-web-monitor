...
from .db_utils import get_optimized_connection, retry_on_db_lock
...
def init_db():
    ...
    if mode and mode[0].lower() == "wal":
        ...
        log.warning(
            f"Failed to set database journal mode to WAL. Current mode: {mode[0] if mode else 'unknown'}"
        )

    log.info(
        "Performing one-time database schema validation and upgrades. This may take a long time on large databases..."
    )
    ...
    if cursor.fetchone():
        ...
        if "duration_ms" not in columns:
            log.info(
                "Upgrading 'events' table: Adding 'duration_ms' column for latency tracking..."
            )
    ...
    cursor.execute(
        "SELECT 1 FROM sqlite_master WHERE type='index' AND name='idx_events_financial_traffic'"
    )
    if not cursor.fetchone():
        log.info(
            "Creating financial traffic index for earnings calculations. This may take a while..."
        )
        cursor.execute(
            "CREATE INDEX idx_events_financial_traffic ON events (node_name, satellite_id, timestamp, status, action);"
        )
    ...
    if not cursor.fetchone():
        ...
        cursor.execute(
            "CREATE INDEX idx_events_latency ON events (node_name, timestamp, duration_ms) WHERE duration_ms IS NOT NULL;"
        )
    ...
    if not cursor.fetchone():
        ...
        cursor.execute(
            "CREATE INDEX idx_storage_earnings ON storage_snapshots (node_name, timestamp DESC, used_bytes);"
        )
    ...
    """)
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_earnings_node_time ON earnings_estimates (node_name, timestamp);"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_earnings_satellite ON earnings_estimates (satellite);"
    )
    cursor.execute(
    ...
    """)
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_payout_node_time ON payout_history (node_name, timestamp);"
    )
...
def blocking_write_hashstore_log(db_path: str, stats_dict: dict) -> bool:
    ...
    try:
        with get_optimized_connection(db_path, timeout=DB_CONNECTION_TIMEOUT) as conn:
            ...
            conn.commit()
        log.info(
            f"Successfully wrote hashstore log for {stats_dict['node_name']}:{stats_dict['satellite']}:{stats_dict['store']}."
        )
        return True
    except Exception:
    ...
    for e in events:
        ...
        data_to_insert_extend(
            (
                ...
                e["error_reason"],
                e["node_name"],
                e.get("duration_ms"),  # Phase 2.1: Include duration
            )
        )
    ...
    with get_optimized_connection(DATABASE_FILE, timeout=DB_CONNECTION_TIMEOUT) as conn:
        ...
        for node_name in node_names:
            ...
            if stats and stats["dl_s"] is not None:
                ...
                conn.commit()
                log.info(
                    f"[AGGREGATOR] Wrote hourly stats for node '{node_name}' at {hour_start_iso}."
                )


def blocking_db_prune(
    db_path,
    events_retention_days,
    hashstore_retention_days,
    earnings_retention_days=None,
    alerts_retention_days=None,
    insights_retention_days=None,
    ...
    with get_optimized_connection(db_path, timeout=DB_CONNECTION_TIMEOUT) as conn:
        ...
        if count > 0:
            log.warning(
                f"Deleting {count} old event(s) from the database. This might take a while..."
            )
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM hashstore_compaction_history WHERE last_run_iso < ?",
        ...
        if count > 0:
            ...
            cursor.execute(
                "DELETE FROM hashstore_compaction_history WHERE last_run_iso < ?",
                (hashstore_cutoff_iso,),
            )
            conn.commit()
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM earnings_estimates WHERE timestamp < ?", (earnings_cutoff_iso,)
        )
        count = cursor.fetchone()[0]
        ...
        if count > 0:
            ...
            cursor.execute(
                "DELETE FROM earnings_estimates WHERE timestamp < ?", (earnings_cutoff_iso,)
            )
            conn.commit()
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM analytics_baselines WHERE last_updated < ?",
            (analytics_cutoff_iso,),
        )
        count = cursor.fetchone()[0]
        ...
        if count > 0:
            ...
            cursor.execute(
                "DELETE FROM analytics_baselines WHERE last_updated < ?", (analytics_cutoff_iso,)
            )
            conn.commit()
...
def get_historical_stats(view: list[str], all_nodes_state: dict[str, Any]) -> list[dict]:
    ...
    with get_optimized_connection(DATABASE_FILE, timeout=DB_CONNECTION_TIMEOUT) as conn:
        ...
        if nodes_for_hist:
            ...
            """,
                (*nodes_for_hist, HISTORICAL_HOURS_TO_SHOW),
            ).fetchall()
        else:
            raw_hist_stats = []
...
def blocking_get_historical_performance(
    events: list[dict[str, Any]], points: int, interval_sec: int
) -> list[dict[str, Any]]:
    import time

    log.info(
        f"Processing {len(events)} in-memory events for historical performance graph generation."
    )
    ...
    recent_events = [
        e
        for e in events
        if e.get("ts_unix", 0) >= cutoff_unix
        and e.get("ts_unix", 0) < last_full_bin_unix + interval_sec
    ]
        ...
        sparse_results.append(
            {
                ...
                "concurrency": round(data["total_ops"] / interval_sec, 2)
                if interval_sec > 0
                else 0,
    ...
    filled_results = _zero_fill_performance_data(
        sparse_results, cutoff_unix, last_full_bin_unix, interval_sec
    )
    log.info(
        f"Returning {len(filled_results)} historical performance data points from in-memory events."
    )
    return filled_results


def _zero_fill_performance_data(
    sparse_data: list[dict], start_unix: float, end_unix: float, interval_sec: int
) -> list[dict]:
    ...
    results_map = {
        int(datetime.datetime.fromisoformat(r["timestamp"]).timestamp() / interval_sec)
        * interval_sec: r
    ...
    while current_bucket_unix <= end_bucket_unix:
        if current_bucket_unix in results_map:
            ...
            filled_results.append(
                {
                    "timestamp": datetime.datetime.fromtimestamp(
                        current_bucket_unix, tz=datetime.timezone.utc
                    ).isoformat(),
                    "ingress_mbps": 0,
                    "egress_mbps": 0,
                    "ingress_bytes": 0,
...
def blocking_get_aggregated_performance(
    node_names: list[str], time_window_hours: int
) -> list[dict[str, Any]]:
    if not node_names:
        return []
    log.info(
        f"Fetching AGGREGATED performance for nodes {node_names} (last {time_window_hours} hours)."
    )
    ...
    with get_optimized_connection(DATABASE_FILE, timeout=DB_CONNECTION_TIMEOUT) as conn:
        ...
        for row in cursor.execute(query, params).fetchall():
            ...
            iso_ts = (
                row_dict.get("time_bucket_start_iso")
                or datetime.datetime.fromtimestamp(ts_unix, tz=datetime.timezone.utc).isoformat()
            )
            sparse_results.append(
                {
                    "timestamp": iso_ts,
                    "ingress_mbps": round(
                        (row_dict.get("ingress_bytes", 0) * 8) / (actual_bin_sec * 1e6), 2
                    ),
                    "egress_mbps": round(
                        (row_dict.get("egress_bytes", 0) * 8) / (actual_bin_sec * 1e6), 2
                    ),
                    ...
                    "concurrency": round(row_dict.get("total_ops", 0) / actual_bin_sec, 2)
                    if actual_bin_sec > 0
                    else 0,
                    "total_ops": row_dict.get("total_ops", 0),
                    "bin_duration_seconds": actual_bin_sec,
                }
            )

    filled_results = _zero_fill_performance_data(
        sparse_results, start_time.timestamp(), now_dt.timestamp(), actual_bin_sec
    )
    log.info(
        f"Returning {len(filled_results)} aggregated performance data points for nodes {node_names}."
    )
    return filled_results
...
def blocking_backfill_hourly_stats(node_names: list[str]):
    ...
    with get_optimized_connection(DATABASE_FILE, timeout=DB_CONNECTION_TIMEOUT) as conn:
        ...
        if last_aggregated_hour:
            ...
            log.info(
                f"[BACKFILL] Last aggregated hour found: {start_from_iso}. Backfilling from this point."
            )
        else:
            ...
            log.info(
                f"[BACKFILL] No existing hourly stats. Backfilling from earliest event hour: {start_from_iso}"
            )
        ...
        if stats_to_insert:
            ...
            log.info(
                f"[BACKFILL] Wrote/updated {cursor.rowcount} hourly stat records in the database."
            )
...
def load_initial_state_from_db(nodes_config: dict[str, dict[str, Any]]):
    ...
    cutoff_datetime = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(
        minutes=load_window_minutes
    )

    with get_optimized_connection(
        DATABASE_FILE, timeout=DB_CONNECTION_TIMEOUT, read_only=True
    ) as conn:
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        ...
        for node_name in nodes_config:
            ...
            log.info(
                f"Loading recent events for node '{node_name}' (last {load_window_minutes} min, max 10000 events)"
            )
            cursor.execute(
                """SELECT * FROM events
...
def blocking_batch_write_hashstore_ingest(db_path: str, records: list[dict]):
    ...
    try:
        with get_optimized_connection(db_path, timeout=DB_CONNECTION_TIMEOUT) as conn:
            ...
            conn.commit()
        log.info(
            f"Successfully ingested {len(records)} hashstore compaction records into the database."
        )
    except Exception:
...
def blocking_get_reputation_history(
    db_path: str, node_name: str, satellite: Optional[str] = None, hours: int = 24
) -> list[dict[str, Any]]:
    """
...
def blocking_write_storage_snapshot(db_path: str, snapshot: dict[str, Any]) -> bool:
    ...
    try:
        ...
        log.info(
            f"Successfully wrote storage snapshot for {snapshot['node_name']} (source: {source})"
        )
        return True
    except Exception:
...
def blocking_get_storage_history(
    db_path: str, node_name: str, days: int = 7
) -> list[dict[str, Any]]:
    """
...
def blocking_get_alert_history(
    db_path: str, node_name: str, hours: int = 24, include_resolved: bool = True
) -> list[dict[str, Any]]:
...
def blocking_get_insights(
    db_path: str, node_names: list[str] = None, hours: int = 24
) -> list[dict[str, Any]]:
    """Get recent insights."""
    try:
        cutoff = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=hours)
...
def blocking_update_baseline(
    db_path: str, node_name: str, metric_name: str, window_hours: int, stats: dict[str, float]
) -> bool:
...
def blocking_get_latest_storage(db_path: str, node_names: list[str]) -> list[dict[str, Any]]:
    """
    ...
    try:
        with get_optimized_connection(db_path, timeout=DB_CONNECTION_TIMEOUT) as conn:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT COUNT(*) FROM storage_snapshots WHERE node_name IN ({})".format(
                    ",".join("?" * len(node_names))
                ),
                node_names,
            )
            count = cursor.fetchone()[0]
            log.info(
                f"blocking_get_latest_storage: Found {count} storage snapshot(s) for nodes {node_names}"
            )
            ...
            if results:
                for r in results:
                    log.info(
                        f"  Node: {r['node_name']}, Available: {r.get('available_bytes', 0) / (1024**4):.2f} TB, Timestamp: {r['timestamp']}"
                    )
            return results
    except Exception:
...
def blocking_get_latest_storage_with_forecast(
    db_path: str, node_names: list[str], days_window: int = 7
) -> list[dict[str, Any]]:
    """
    ...
    try:
        ...
        for storage in latest_storage:
            ...
            for days in time_windows:
                ...
                log.debug(
                    f"[{node_name}] {days}d window: {growth_rates[f'{days}d']['growth_rate_gb_per_day']} GB/day "
                    f"using {n} points"
                )
...
def blocking_write_earnings_estimate(db_path: str, estimate: dict[str, Any]) -> bool:
    ...
    try:
        with get_optimized_connection(db_path, timeout=DB_CONNECTION_TIMEOUT) as conn:
            ...
            conn.commit()
        log.info(
            f"Successfully wrote earnings estimate for {estimate['node_name']}/{estimate['satellite']}"
        )
        return True
    except Exception:
...
def blocking_get_latest_earnings(
    db_path: str, node_names: list[str], period: str = None
) -> list[dict[str, Any]]:
    """
        ...
        with get_optimized_connection(db_path, timeout=DB_CONNECTION_TIMEOUT) as conn:
            ...
            if results:
                ...
                for node in node_names:
                    ...
                    log.info(
                        f"[blocking_get_latest_earnings]   {node}: {len(node_records)} satellites"
                    )
            else:
                log.warning(
                    f"[blocking_get_latest_earnings] No results found for nodes {node_names}, period {period}"
                )
...
def blocking_write_payout_history(db_path: str, payout: dict[str, Any]) -> bool:
    ...
    try:
        with get_optimized_connection(db_path, timeout=DB_CONNECTION_TIMEOUT) as conn:
            ...
            conn.commit()
        log.info(
            f"Successfully wrote payout history for {payout['node_name']}/{payout['satellite']}"
        )
        return True
    except Exception:
...
def blocking_get_payout_history(
    db_path: str, node_names: list[str] = None, satellite: str = None, months: int = 12
) -> list[dict[str, Any]]:
    """
