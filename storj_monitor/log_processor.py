...
def parse_log_line(line: str, node_name: str, geoip_reader, geoip_cache: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        action, size, piece_id, sat_id, remote_addr = (
            log_data.get("Action"),
            log_data.get("Size"),
            log_data.get("Piece ID"),
            log_data.get("Satellite ID"),
            log_data.get("Remote Address"),
        )
    ...
    try:
        while True:
            ...
            if parsed["type"] == "operation_start":
                ...
                log.debug(
                    f"[{node_name}] Stored operation_start: action={parsed['action']}, piece={parsed['piece_id'][:16]}..., sat={parsed['satellite_id'][:12]}..."
                )
                    ...
                    if current_time - last_storage_sample_time >= STORAGE_SAMPLE_INTERVAL:
                        ...
                        if (
                            ...
                            try:
                                ...
                                log.info(
                                    f"[{node_name}] Sampled storage from logs: {available_space / (1024**4):.2f} TB available"
                                )
                            except Exception as e:
                                log.error(
                                    f"[{node_name}] Failed to write storage snapshot from logs: {e}"
                                )
                ...
                if len(operation_start_times) > MAX_TRACKED_OPERATIONS:
                    ...
                    log.warning(
                        f"[{node_name}] operation_start_times exceeded {MAX_TRACKED_OPERATIONS}, removed {to_remove} oldest entries"
                    )
                continue
            ...
            if parsed["type"] == "traffic_event":
                ...
                if event["duration_ms"] is None:
                    ...
                    if start_data is not None:
                        ...
                        if arrival_duration_ms > 4000:
                            ...
                            log.debug(
                                f"[{node_name}] Used timestamp fallback: {timestamp_duration_ms}ms (arrival suggested {arrival_duration_ms}ms) for {event['action']}"
                            )
                        else:
                            # Normal case: use arrival_time for best precision
                            event["duration_ms"] = arrival_duration_ms
                            log.debug(
                                f"[{node_name}] Calculated duration: {arrival_duration_ms}ms for {event['action']}"
                            )
                    else:
                ...
                if app_state["db_write_queue"].full():
                    log.warning(
                        "Database write queue is full. Pausing log processing to allow DB to catch up."
                    )
        ...
        while not shutdown_event.is_set():
            ...
            try:
                ...
                if st.st_ino != current_inode:
                    log.warning(
                        f"Log rotation by inode change detected for '{log_path}'. Re-opening."
                    )
                    f.close()
                    f = None
                    continue
                ...
                log.error(
                    f"Error checking log status for '{log_path}': {e}. Re-opening.", exc_info=True
                )
                f.close()
                f = None
            ...
            while True:
                ...
                if len(parts) == 2:
                    try:
                        ...
                        log.warning(
                            f"[{node_name}] Received malformed line from {host}:{port}, ignoring: {line[:100]}"
                        )
                else:
                    log.warning(
                        f"[{node_name}] Received malformed line from {host}:{port}, ignoring: {line[:100]}"
