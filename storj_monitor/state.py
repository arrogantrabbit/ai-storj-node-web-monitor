...
class IncrementalStats:
    ...
    def get_or_create_satellite(self, sat_id: str) -> Dict[str, int]:
        ...
        if sat_id not in self.satellites:
            self.satellites[sat_id] = {
                ...
                "total_get_repair_size": 0,
                "total_put_repair_size": 0,
            }
    ...
    def to_payload(self, historical_stats: list[dict] = None) -> dict[str, any]:
        ...
        satellites = sorted(
            [{"satellite_id": k, **v} for k, v in self.satellites.items()],
            key=lambda x: x["uploads"] + x["downloads"],
            reverse=True,
        )
        all_buckets = [
            "< 1 KB",
        ...
        transfer_sizes = [
            {
                ...
                "downloads_success_size": self.dls_success_size[b],
                "downloads_failed_size": self.dls_failed_size[b],
                "uploads_success_size": self.uls_success_size[b],
                "uploads_failed_size": self.uls_failed_size[b],
            }
            for b in all_buckets
        ]
        sorted_errors = sorted(
            self.error_agg.items(), key=lambda item: item[1]["count"], reverse=True
        )
        final_errors = []
            ...
            if "placeholders" in data:
                for ph_data in data["placeholders"]:
                    if ph_data["type"] == "number":
                        ...
                        range_str = (
                            str(min_val) if min_val == max_val else f"({min_val}..{max_val})"
                        )
        ...
        top_pieces = [
            ...
            for k, v in heapq.nlargest(
                10, self.hot_pieces.items(), key=lambda item: item[1]["count"]
            )
        ]
        return {
            "type": "stats_update",
            ...
            "top_countries_dl": [
                {"country": k, "size": v} for k, v in self.countries_dl.most_common(10) if k
            ],
            "top_countries_ul": [
                {"country": k, "size": v} for k, v in self.countries_ul.most_common(10) if k
            ],
        }
