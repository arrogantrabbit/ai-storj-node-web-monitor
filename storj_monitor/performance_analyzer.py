...
def calculate_percentiles(values: list[float], percentiles: list[int] = None) -> dict[str, float]:
    ...
    for p in percentiles:
        ...
        if rank < 1:
            ...
            result[f"p{p}"] = sorted_values[lower_idx] + fraction * (
...
def analyze_latency_data(events: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    """
    # Group by category
    latencies_by_category = {"get": [], "put": [], "audit": [], "all": []}
...
def detect_slow_operations(
    events: list[dict[str, Any]], threshold_ms: int = 5000, limit: int = 10
) -> list[dict[str, Any]]:
    """
...
def blocking_get_latency_stats(
    db_path: str, node_names: list[str], hours: int = 1
) -> dict[str, Any]:
    """
        ...
        with get_optimized_connection(db_path, timeout=DB_CONNECTION_TIMEOUT) as conn:
            ...
            for diag in diagnostics:
                ...
                if total > 0:
                    ...
                    log.info(
                        f"[{node}] Latency data: {with_duration}/{total} events ({percentage:.1f}%) have duration_ms"
                    )
                    if with_duration == 0 and total > 100:
                        log.warning(
                            f"[{node}] No duration data available - check if Storj node logs include duration field or enable DEBUG logging"
                        )
...
def blocking_get_latency_histogram(
    db_path: str, node_names: list[str], hours: int = 1, bucket_size_ms: int = 100
) -> list[dict[str, Any]]:
    """
    ...
    try:
        ...
        with get_optimized_connection(
            db_path, timeout=DB_CONNECTION_TIMEOUT, read_only=True
        ) as conn:
            placeholders = ",".join("?" for _ in node_names)
