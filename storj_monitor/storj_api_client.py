...
class StorjNodeAPIClient:
                ...
                if not self.is_available:
                    ...
                    try:
                        ...
                        if data and "nodeID" in data:
                            ...
                            log.warning(
                                f"[{self.node_name}] API health check failed - invalid response"
                            )
                            self.is_available = False
                            self._connection_state = "error"
                    except Exception as e:
    ...
    async def _get(self, path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with self.session.get(url, allow_redirects=True, max_redirects=2) as resp:
                ...
                if resp.status == 200:
                    ...
                    if json_data is None:
                        ...
                        log.debug(
                            f"[{self.node_name}] API returned JSON dict with {len(json_data)} keys"
                        )
                    elif isinstance(json_data, list):
                        log.debug(
                            f"[{self.node_name}] API returned JSON list with {len(json_data)} items"
                        )
                    else:
            ...
            log.error(
                f"[{self.node_name}] Unexpected error in API request for {path}: {e}", exc_info=True
            )
            self.is_available = False
            self._connection_state = "error"
            return None
...
async def auto_discover_api_endpoint(node_config: Dict[str, Any]) -> Optional[str]:
    ...
    if node_type == "file":
        ...
        if host:
            ...
            log.info(f"[{node_name}] Attempting auto-discovery for remote node at {host}...")
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(f"{endpoint}/api/sno") as resp:
                    if resp.status == 200:
                        ...
                        if data and "nodeID" in data:
                            log.info(f"[{node_name}] Auto-discovered API endpoint: {endpoint}")
                            return endpoint
        except Exception:
...
async def setup_api_client(
    app: Dict[str, Any], node_name: str, api_endpoint: str
